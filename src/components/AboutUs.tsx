...
interface AboutUsProps {
  isFullPage?: boolean;
  showCoreValuesAndCTA?: boolean;
}

export function AboutCoreValuesAndCTA() {
  return (
...
export default function AboutUs({ isFullPage = false, showCoreValuesAndCTA = true }: AboutUsProps) {
    ...
    async function fetchStatsAndPage() {
      try {
        const [{ count: catCount }, { count: prodCount }, { data: pageData }] = await Promise.all([
          supabase.from('categories').select('*', { count: 'exact', head: true }).eq('tenant_id', '00000000-0000-0000-0000-000000000002'),
          supabase.from('products').select('*', { count: 'exact', head: true }).eq('tenant_id', '00000000-0000-0000-0000-000000000002'),
          supabase.from('pages').select('*').eq('tenant_id', '00000000-0000-0000-0000-000000000002').or('slug.eq.gioi-thieu,slug.eq.about').maybeSingle()
        ]);
  ...
  return (
    <div className="w-full bg-white">
      ...
      <section className="py-20 lg:py-24 border-b border-slate-100 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
            ...
            {/* Left Column - Image Container (100% pure image, no overlay box) */}
            <motion.div 
            ...
            >
              <div className="relative h-[450px] md:h-[550px] w-full rounded-2xl overflow-hidden shadow-2xl border border-slate-100 bg-slate-900">
                <img 
                  src={settings?.aboutImageUrl || customPage?.image_url || "https://images.unsplash.com/photo-1541888946425-d0fbb186c5f8?q=80&w=1000&auto=format&fit=crop"} 
                  alt={settings.companyName || "Vật tư công trình xây dựng Sbuild"}
                  className="w-full h-full object-cover transition-transform duration-700 hover:scale-105"
                  onError={(e) => {
                    const target = e.currentTarget;
                    if (target.src !== '/images/hero-banner.webp') {
                      target.src = '/images/hero-banner.webp';
                    }
                  }}
                  loading="eager"
                />
      ...
      {/* Core Values & CTA Sections (Only rendered when showCoreValuesAndCTA is true) */}
      {showCoreValuesAndCTA && (
        <>
          {/* Core Values Section */}
          <section className="py-20 bg-slate-50/60 border-b border-slate-100">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
